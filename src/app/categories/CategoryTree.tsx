  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-80">
        <h3 className="font-semibold text-slate-800 mb-1">קטגוריה חדשה</h3>
        ...
        <div className="flex gap-2">
          <button
            onClick={submit}
            disabled={loading}
            className="flex-1 px-4 py-2 bg-[#185FA5] text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50"
          >
            {loading ? 'שומר...' : 'הוסף'}
          </button>
          <button
              ...
              {initialCategories.length === 0 ? (
                ...
                <tr key={cat.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    ...
                    >
                      {cat.is_active !== false ? (
                        <><HiCheck className="w-3 h-3" /> פעיל</>
                      ) : (
                        'כבוי'
                      )}
                    </button>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      ...
                      <button
                        ...
                        title={cat.video_count > 0 ? 'לא ניתן למחוק קטגוריה עם סרטונים' : 'מחק'}
                        disabled={cat.video_count > 0 || isPending}
