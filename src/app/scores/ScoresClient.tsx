...
function ScorePill({ value }: { value: number | null }) {
  ...
  const color = value >= 70 ? 'bg-green-100 text-green-800' : value >= 40 ? 'bg-amber-100 text-amber-800' : 'bg-red-100 text-red-800'
  return <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${color}`}>{value}</span>
}

function PenalizeModal({ userId, onClose, onDone }: { userId: string; onClose: () => void; onDone: () => void }) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-80">
        ...
        <div className="flex gap-2">
          <button onClick={confirm} disabled={loading} className="flex-1 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 disabled:opacity-50">
            {loading ? 'שולח...' : 'הענש'}
          </button>
          <button onClick={onClose} className="flex-1 py-2 bg-slate-100 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-200">ביטול</button>
        </div>
      </div>
    </div>
  )
...
function VideoScoresTab({ rows }: { rows: VideoScoreRow[] }) {
  ...
  const filtered = rows.filter((r) => {
    ...
    return (r.video_title?.toLowerCase().includes(q) ?? false) ||
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {paged.length === 0 ? (
              <tr><td colSpan={8} className="py-12 text-center text-slate-400 text-sm">אין תוצאות</td></tr>
            ) : paged.map((r, i) => (
...
function CreatorScoresTab({ rows }: { rows: CreatorScoreRow[] }) {
    ...
    <div>
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {paged.length === 0 ? (
              <tr><td colSpan={8} className="py-12 text-center text-slate-400 text-sm">אין נתונים</td></tr>
            ) : paged.map((r, i) => (
              <tr key={r.user_id ?? i} className="hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3 text-slate-400 text-xs">{(page - 1) * PAGE_SIZE + i + 1}</td>
                <td className="px-4 py-3">
                  <p className="font-medium text-slate-700 text-sm">{r.profile?.display_name || r.profile?.username || '—'}</p>
                  {r.profile?.username && <p className="text-xs text-slate-400">@{r.profile.username}</p>}
                </td>
                <td className="px-4 py-3"><ScorePill value={r.score} /></td>
                <td className="px-4 py-3 text-slate-600 text-xs">{r.avg_video_score ?? '—'}</td>
                <td className="px-4 py-3 text-slate-600 text-xs">{r.follower_growth != null ? `${r.follower_growth > 0 ? '+' : ''}${r.follower_growth}` : '—'}</td>
                <td className="px-4 py-3 text-slate-600 text-xs">{r.engagement_rate != null ? `${(r.engagement_rate * 100).toFixed(1)}%` : '—'}</td>
                <td className="px-4 py-3">
                  {(r.violation_count ?? 0) > 0 ? (
                    <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-medium">{r.violation_count}</span>
      ...
      {penalizingId && (
        <PenalizeModal
          ...
          onDone={() => { setPenalizingId(null); startTransition(() => router.refresh()) }}
        />
      )}
    </div>
  )
...
export default function ScoresClient({
  ...
  return (
    <div>
      ...
      {activeTab === 'creators' && <CreatorScoresTab rows={creatorScores} />}
